...
- [ ] All private instance fields use camelCase with underscore prefix (`_fieldName`)
- [ ] All static readonly immutable fields (collections, dictionaries, arrays, Regex) use PascalCase
- [ ] All static mutable fields use camelCase with underscore prefix (`_fieldName`)
- [ ] All parameters use camelCase
- [ ] All primary constructor parameters use camelCase (classes/structs) or PascalCase (records)
- [ ] All local variables use camelCase
- [ ] All async methods end with `Async` suffix (except controller actions - see exception)
- [ ] All interfaces start with `I` prefix
- [ ] All attributes end with `Attribute` suffix
- [ ] All enum types use singular noun (non-flags) or plural noun (flags)
- [ ] All DTOs use `Dto` suffix (not `DTO`)
- [ ] All extension method classes end with `Extensions` suffix
- [ ] All file names match class/interface names exactly
- [ ] All namespaces match folder structure
- [ ] All boolean properties use `Is`, `Has`, or `Can` prefix
- [ ] All constants use PascalCase
- [ ] All generic type parameters use PascalCase (typically `T` for single parameter, `TName` for descriptive)
- [ ] All controller actions are async (Async suffix optional)
- [ ] All service/repository async methods have `Async` suffix
- [ ] All cancellation tokens use `cancellationToken` parameter name
- [ ] No identifiers contain two consecutive underscores (`__`)
